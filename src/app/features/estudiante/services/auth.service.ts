...
export class AuthService {
    ...
    return this.http.post<Respuesta<AuthRespuesta>>(this.apiUrl, request).pipe(
      tap(res => {
        if (!res.blnError && res.valorRetorno) {
          ...
          localStorage.setItem(this.ROLE_KEY, data.role === 'Administrador' ? 'admin' : 'estudiante');
  ...
  getRole(): 'admin' | 'estudiante' | null {
    return localStorage.getItem(this.ROLE_KEY) as 'admin' | 'estudiante' | null;
  }
